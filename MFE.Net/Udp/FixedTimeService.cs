...
namespace MFE.Net.Udp
{
    ...
    public static class FixedTimeService
    {
        ...
        public static DateTime GetTimeFromNTP(byte[] serverAddress)
        {
            ...
            try
            {
                ...
                if (ntpSocket.Poll(timeout * 1000 * 1000, SelectMode.SelectRead))
                {
                    ...
                    //Debug.Print("NTP time:" + ntpTime.ToString());  

                    ntpTime = ntpTime.AddMinutes(timeZoneOffset);  // universal => local
                    //Debug.Print("Zone time:" + ntpTime.ToString());
        ...
        private static bool IsDST(DateTime today)
        {
            ...
            {   // northern hem
                if (DateTime.Now < dstStartDay)
                    return false; // before dsl
                else if (DateTime.Now >= dstEndDay)
            ...
            {   // southern hem
                if (DateTime.Now < dstEndDay)
                    return true; // still dsl
                else if (DateTime.Now >= dstStartDay)
